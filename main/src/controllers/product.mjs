...
let self = ({
  ...
  autoChangePrices: async function(req, res, next) {
    let offset = 0;
    ...
    }, "_id title sources combinations", async function(err, products) {
      res.json(products);
    });
  },
  ...
  modifyproducts: async function(req, res, next) {
    ...
    }, "_id title theCategories categories firstCategory ", async function(err, products) {
      // console.log('modify products and', products.length);
      var cats = [];
      await _.forEach(products, async (p, cx) => {

        if (p.theCategories)
          await _.forEach(p.theCategories, async (c, cx) => {
            // console.log('c', c);
            if (c.name != "بلک فرآیدی" && c.name != "فروش فوق العاده" && c.name != "آفباکس")
              await self.getCategory(c).then(async (t) => {
                // console.log('t',t);
                // res.json({
                //     t: t,
                //     product: products
                // })
                // console.log('going to change p._id', p._id, t);
                // console.log('going to change p._id',p._id);
                Product.findByIdAndUpdate(p._id, t, {
                  "fields": { "_id": 1 }
                }, function(err, product) {
                  // console.log('product changed... ', product._id, t);
  ...
  getCategory: function(c, query = { "name.fa": c.name }) {
    return new Promise(function(resolve, reject) {
      Category.findOne(query, function(err, category) {
        // console.log('search for...', c, query);

        if (err || !category) {
          // console.log('err', err);
          if (query["name.fa"] == c.name)
            self.getCategory(c, {
              "name.fa": {
                $exists: true,
                "$regex": c.name,
        ...
        if (category.parent) {
          // console.log('find cat', category);
          self.getCategoryById(category).then((cvgh) => {
            resolve(cvgh);
            // console.log(cvgh);
            // console.log(cvgh);
          });
  ...
  getCategoryById: function(c) {
    return new Promise(function(resolve, reject) {
      // console.log('getCategoryById');
      Category.findById(c.parent
        , function(err, category) {

          if (!category.parent) {
            // console.log('category', category.name, c.name);

            resolve({
              firstCategory: category._id,
              secondCategory: c._id
            });
          }
          if (category.parent) {
            Category.findById(category._id, function(err, thecat) {
              if (!thecat.parent) {
                if (c._id == category._id && category._id == thecat._id) {
                  resolve({
                    firstCategory: thecat._id
                  });
                } else {
                  resolve({
                    firstCategory: thecat._id,
                    secondCategory: category._id,
                    thirdCategory: c._id
                  });
                }
              } else {
                Category.findById(thecat.parent, function(err, thecat2) {
                  if (!thecat2.parent) {
                    // console.log(thecat2.name);
                    // console.log(thecat.name);
                    // console.log(category.name);
                    // console.log(c.name);
                    if (thecat._id.toString() == category._id.toString()) {
                      // console.log('equal');
                      if (c._id == thecat._id && thecat2._id == thecat._id) {
                        resolve({
                          firstCategory: thecat2._id
                        });
                      } else
                        resolve({
                          firstCategory: thecat2._id,
                          secondCategory: thecat._id,
                          thirdCategory: c._id
                        });
                    } else {
                      // console.log(thecat._id);
                      // console.log(category._id);
                      resolve({
                        firstCategory: thecat2._id,
                        secondCategory: thecat._id,
                        thirdCategory: category._id,
                        fourthCategory: c._id
                      });
                    }
                  } else {
                    reject({
  ...
  importproducts: function(req, res, next) {
    res.json({
      success: true
    });
  ...
  importproducts2:
    ...
    async function(req, res, next) {
      ...
      // fs.readFile('../hs.json', 'utf8', function (err, data) {
      ...
      //             // console.log('value', key, parsedBody[key].title);
      //
      //             let productt = {
      //                 title: {
      //                     'fa': parsedBody[key].title || null,
      //                 },
      //                 price: parsedBody[key].price || null,
      //                 salePrice: parsedBody[key].sale_price || null,
      //                 options: parsedBody[key].options || [],
      //                 combinations: parsedBody[key].combinations || [],
      //                 sections: parsedBody[key].sections || [],
      //                 theCategories: parsedBody[key].categories,
      //                 photos: [],
      //                 status: 'published'
      //             };
      //
      //             if (parsedBody[key].images) {
      //                 _.forEach((parsedBody[key].images ? parsedBody[key].images : []), async (c, cx) => {
      //                     // console.log('images[', cx, ']', c.src);
      //                     let mainUrl = encodeURI(c.src);
      //                     let filename =
      //                             c.src.split('/').pop(),
      //                         name = (getFormattedTime() + filename).replace(/\s/g, ''),
      //                         type = path.extname(name),
      //                         mimtype = mime.getType(type),
      //                         filePath = path.join(__dirname, "../public/customer/", name),
      ...
      //
      //                                 productt.photos.push(media.url);
      //                                 if (productt.photos.length == parsedBody[key].images.length) {
      //                                     Product.create(productt, function (err, product) {
...
// console.log('req.params',req.query.include);
  ...
  editAdmin: function(req, res, next) {
    ...
    }).populate("customer").select("-views");
  }
  ,
