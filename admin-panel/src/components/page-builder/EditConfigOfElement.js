...
const EditConfigOfElement = (props) => {
  ...
  const handleChangeTextComplete = (e,er) => {
    // er.defaultValue=color.hex;
    console.clear();
    console.log('handleChangeTextComplete',e.target.value,er);
    // const { id, dropzoneID, parentID } = props;
    setState({...setState,
  ...
  const handleChangeSelectComplete = (e,er) => {
    ...
    state.updateElement(id, dropzoneID, parentID, {
     ...
     countOfCols: theState.countOfCols }
    });
    // setState({ background: color.hex });
    ...
    defaultForm.map((er, r) => {
      ...
      if (er.type === "color") {
        return <div className={'row'}><Color label={er.label} color={er.defaultValue || "#ccc"}
                      onChangeComplete={(e)=>{handleChangeComplete(e,er)}}
        /></div>;
      }else if (er.type === "select-options") {
        return <select defaultValue={er.defaultValue}
                       onChange={(e)=>{handleChangeSelectComplete(e,er)}}
        >
          {er.children.map(ch=><option value={ch.value}>{ch.name}</option>)}
        </select>;
      }else if (er.type === "text") {
        return <input defaultValue={er.defaultValue} name={er.name} placeholder={er.label} onChange={(e)=>{handleChangeTextComplete(e,er)}}/>;
      } else
