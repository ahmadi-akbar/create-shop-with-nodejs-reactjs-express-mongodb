...
import { TextInput,NumberInput, useTranslate ,ArrayInput,SimpleFormIterator,FormDataConsumer,SelectInput} from "react-admin";
import {
  CatRefField,
  FileChips,
  getStockStatus,
...
export default (props) => {
  let { record, counter, updater } = props;
  const translate = useTranslate();
  let combs = useWatch({ name: "combinations" });
  const {setValue}=useFormContext();
  let [ST, setST] = React.useState(StockStatus() || []);
